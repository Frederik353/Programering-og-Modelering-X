    ...
    def calculate_single_body_acceleration(self, body_index):
        ...
        for index, external_body in enumerate(self.bodies):
            if index != body_index:
                ...
                k1.x = tmp * (external_body.location.x -
                              target_body.location.x)
                k1.y = tmp * (external_body.location.y -
                              target_body.location.y)
                k1.z = tmp * (external_body.location.z -
                ...
                tmp_loc = partial_step(
                    target_body.location, tmp_vel, 0.5 * self.time_step)
                ...
                tmp_loc = partial_step(
                    target_body.location, tmp_vel, self.time_step)
...
def plot_output(bodies, outfile=None):
    ...
    for current_body in bodies:
        max_dim = max(max(current_body["x"]), max(
            current_body["y"]), max(current_body["z"]))
...
mercury = {"location": point(0, 5.0e10, 0),
           "mass": 3.285e23, "velocity": point(47000, 0, 0)}
venus = {"location": point(0, 1.1e11, 0), "mass": 4.8e24,
         "velocity": point(35000, 0, 0)}
earth = {"location": point(0, 1.5e11, 1e10), "mass": 6e24,
         "velocity": point(30000, 0, 0)}
mars = {"location": point(0, 2.2e11, 0), "mass": 6.4e23,
        "velocity": point(24000, 0, 0)}
jupiter = {"location": point(0, 7.7e11, 0),
           "mass": 1e28, "velocity": point(13000, 0, 0)}
saturn = {"location": point(0, 1.4e12, 0), "mass": 5.7e26,
          "velocity": point(9000, 0, 0)}
uranus = {"location": point(0, 2.8e12, 0), "mass": 8.7e25,
          "velocity": point(6835, 0, 0)}
neptune = {"location": point(0, 4.5e12, 0),
           "mass": 1e26, "velocity": point(5477, 0, 0)}
pluto = {"location": point(0, 3.7e12, 0), "mass": 1.3e22,
         "velocity": point(4748, 0, 0)}

sat = {"location": point(0, 1.0e10, 0), "mass": 1e23,
       "velocity": point(3e4, 0, 0)}
...
if __name__ == "__main__":
    ...
    bodies = [
        ...
        body(location=mercury["location"], mass=mercury["mass"],
             velocity=mercury["velocity"], name="sun"),
        body(location=venus["location"], mass=venus["mass"],
             velocity=venus["velocity"], name="sun"),
