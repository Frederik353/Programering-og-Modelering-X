...
class Ball(pygame.sprite.Sprite):
    def __init__(self, ball, svw_history, scale, offset_x=0, offset_y=0, trace=True):
        self.scale = scale
        self.id = ball.id
        self._ball = ball
        self.radius = d_to_px(ball.R, self.scale)

        self.svw_history = svw_history
        self.xs = d_to_px(scale, self.svw_history[:,0,0], offset_x)
        self.ys = d_to_px(scale, self.svw_history[:,0,1], offset_y)

        super(Ball, self).__init__()

        self.color = BALL_COLORS.get(self.id, (255,255,255))
...
class AnimateShot(object):
    def __init__(self, shot):
        ...
        self.size = MAX_SCREEN
        self.cloth_color = CLOTH_COLOR
        self.rail_color = RAIL_CLOTH_COLOR
        self.edge_color = EDGE_COLOR

        self.shot = shot
        self.table = shot.table
        self.balls = shot.balls
        self.times = shot.history['time']
        ...
        self.px = {
            'rail': d_to_px(self.scale, self.table.vegg_bredde),
            ...
            'diamond': d_to_px(self.scale, psim.bord_merke_størrelse),
        }
    ...
    def calculate_table_size(self):
        return (
            2*self.table.vegg_bredde + 2*self.table.edge_width + self.table.w,
            2*self.table.vegg_bredde + 2*self.table.edge_width + self.table.l,
        )
    ...
    def init_ball_sprites(self):
        ...
        for ball_id, ball in self.balls.items():
            self.ball_sprites.add(Ball(
                ball=ball,
                svw_history=self.shot.history['balls'][ball_id]["svw"],
                scale=self.scale,
