...
def molarmasse(formel):
    ...
    for index, letter in enumerate(formel): # hvis stor bokstav = nytt element, legg til + tegn hvis tall gange
        # print(formel)
    ...
    for index, letter in enumerate(formel):
        if letter.isupper():
            element = letter + (formel[index + index_offset + 1] if formel[index + index_offset + 1].islower() else "")
